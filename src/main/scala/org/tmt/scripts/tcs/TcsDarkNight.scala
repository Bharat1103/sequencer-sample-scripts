  ...
  cs.handleCommand("setup-tcs") { command =>
    spawn {
      ...
      val firstAssemblyResponse =
        cs.setup("tcs-assembly1", command.withId(Id(s"${command.id}a"))).await
      val commandFailed =
        firstAssemblyResponse.isInstanceOf[CommandResponse.Failed]

      val restAssemblyResponses = if (commandFailed) {
        Set(
          cs.setup("tcs-assembly2", command.withId(Id(s"${command.id}c")))
            .await)
      } else {
        Set(
          cs.setup("tcs-assembly3", command.withId(Id(s"${command.id}b")))
            .await)
      }
