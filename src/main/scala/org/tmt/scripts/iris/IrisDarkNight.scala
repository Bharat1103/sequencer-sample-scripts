...
class IrisDarkNight(cs: CswServices) extends Script(cs) {
  ...
  cs.handleCommand("setup-iris") { command =>
    spawn {
      ...
      val firstAssemblyResponse =
        cs.setup("iris-assembly1", command.withId(Id(s"${command.id}a"))).await
      val commandFailed =
        firstAssemblyResponse.isInstanceOf[CommandResponse.Failed]

      val restAssemblyResponses = if (commandFailed) {
        Set(
          cs.setup("iris-assembly2", command.withId(Id(s"${command.id}c")))
            .await)
      } else {
        Set(
          cs.setup("iris-assembly3", command.withId(Id(s"${command.id}b")))
            .await)
      }
